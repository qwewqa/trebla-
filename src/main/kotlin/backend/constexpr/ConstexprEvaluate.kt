...
import xyz.qwewqa.trebla.backend.compile.FunctionIRNodeVariant.Set
import kotlin.math.*
...
fun IRNode.constexprEvaluate(context: ConstexprEvaluationContext): Double {
    return when (this) {
        is ValueIRNode -> this.value
        is ReadTempIRNode -> context[this.id]
        is AssignTempIRNode -> run { context[this.id] = this.rhs.constexprEvaluate(context); 0.0 }
